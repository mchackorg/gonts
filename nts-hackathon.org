...
necessary to satisfy the unlinkability goals discussed in Section
11.1.
#+END_QUOTE

Cookies are generated by the server, then encrypted by a server-only
master secret. The NTS-KE and NTP servers should share the master
secret.

The server recognizes their own cookies by being able to decrypt them.

TLS session tickets/cookies:
